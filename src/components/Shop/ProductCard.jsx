...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.div 
      ...
      whileHover={{ y: -4 }}
    >
      <div className="relative aspect-[9/10] overflow-hidden bg-gray-50 group">
        <img 
      ...
      <div className="p-2 flex-1 flex flex-col gap-1.5">
        <div className="min-w-0">
           <div className="flex justify-between items-start mb-0.5">
             <span className="text-[9px] uppercase tracking-wider text-gray-500 font-medium truncate leading-none">
                  {product.category || 'Collection'}
             </span>
             {product.is_available && (
                <span className="flex items-center gap-1 text-[8px] font-semibold text-green-700 bg-green-50 px-1.5 py-0.5 rounded border border-green-100 leading-none">
           ...
           <h3 className="text-xs font-bold text-gray-900 leading-tight truncate group-hover:text-orange-600 transition-colors pt-0.5">
                {product.name}
           </h3>
        </div>
        ...
        <button
            ...
            className={`w-full flex items-center justify-center gap-1.5 py-1.5 rounded text-[10px] font-bold uppercase tracking-wide transition-all ${
              product.is_available 
